(function ($) {
    ...
    function modifyCartStaffDisplay(booknetic) {
        ...
        booknetic.cartArr.forEach(function (cartItem, index) {
            ...
            var staffDisplay = cartRow.find('.booknetic-cart-item-body-row').filter(function () {
                return $(this)
                    .find('.booknetic-cart-item-body-cell')
                    .first()
                    .text()
                    .toLowerCase()
                    .includes('staff');
            ...
            if (staffDisplay.length === 0) return;

            var staffName = 'Staff #' + cartItem.staff;

            // Try to fetch staff name from staff step (if exists in DOM)
            var staffCard = panel.find('[data-step-id="staff"] .booknetic_card[data-id="' + cartItem.staff + '"]');
            if (staffCard.length > 0) {
                var nameEl = staffCard.find('.booknetic_card_title, .booknetic_card_name');
                if (nameEl.length > 0) {
                    staffName = nameEl.text().trim();
                }
            }

            var staffHtml = '<strong style="color:#2196F3;">' + staffName + '</strong>';

            var valueCell = staffDisplay.find('.booknetic-cart-item-body-cell').eq(1);
            if (valueCell.length > 0) {
                valueCell.html(staffHtml);
            }
        });
    }
    ...
    function addServiceAssignmentLabels(booknetic) {
        ...
        booknetic.cartArr.forEach(function (cartItem, index) {
            ...
            // If guest service, add guest information (from customer_data if present)
            if (assignedTo === 'guest') {
                var guestInfo = cartItem.customer_data || cartItem.guest_info || {};
                ...
                guestInfoHtml += '<strong>' + (guestInfo.first_name || guestInfo.name || 'N/A') + '</strong><br>';
